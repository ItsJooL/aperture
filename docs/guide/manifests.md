...
Every manifest is a YAML file under `manifests/` (any subdirectory, read recursively) with the same three top-level keys: `apiVersion`, `kind`, and `metadata.name`. It also has a `spec` whose shape depends on `kind`. Eight kinds exist:
...
| [`ApertureConfig`](#apertureconfig) | Tenancy mode, default roles, MCP, and CLI binary name; one per project |
...
`spec.fields` is the only required key. The sections below cover every other key: `tenantScoped`, `scopedBy`, `optimisticLocking`, `softDelete`, `permissions`, `policies`, and `hooks`. See [Entity](/reference/manifest-schema#entity) for the full field table.
...
Use hand-written SQL for backfills and data fixes that do not map to a manifest field change. `rollback` is required; the build fails without it. See [Build & Deploy → Manual migration manifests](/guide/build-deploy#manual-migration-manifests).
...
`unique: true` generates a unique index and is required for encrypted fields you also want to filter on. See [deterministic encryption](/guide/security-audit#field-level-encryption). `index: true` generates a plain, non-unique index for a field you filter or sort on often.

`since:` and `renamedFrom:` support versioned rollout and zero-downtime renames. See [Build & Deploy → Schema automation](/guide/build-deploy#schema-automation). `encrypted:` is covered in [Security & Audit → Field-level encryption](/guide/security-audit#field-level-encryption). The full type table (`string`, `decimal`, `integer`, `boolean`, `uuid`, `datetime`, `ref`, `oneof`) is in [Core Concepts](/guide/core-concepts#field-types) and [the reference](/reference/manifest-schema#spec-fields).
...
Adds an `aperture_tenant_id` column and auto-filters (and auto-injects on create) every query by the caller's tenant, in POOL mode. Use it for core domain entities that belong to one tenant; leave it `false` for shared reference data (countries, currencies, product catalogs). See [Multi-Tenancy](/guide/multi-tenancy) for the full treatment of POOL vs. NONE, tenant-aware foreign keys, and provisioning.

## Scoping by relationship (`scopedBy`)

`scopedBy` partitions an entity's rows by the value of a relationship, carried per-request in a header. It is new, and it is easy to assume it behaves like `tenantScoped`. It does not, so read this section before you use it.
...
Without the header, this collection query returns `200` with an empty page (the scope predicate matches nothing). With it, Elide adds `WHERE task.project.id = :scope` to the query, on top of any tenant filter already in play. By contrast, fetching a single task by id returns `403` without the header. See the fail-closed note above.

### Pair it with ABAC for real per-scope access control

The header narrows *which* partition is queried; it never checks whether the caller *should* see that partition. That is exactly what `policies:` is for. In the example above, `scopedBy: project` picks the partition and `ProjectTeamOnly` (an `AbacPolicy` evaluated against `#user.securityAttributes`) decides who's allowed to query it at all. Neither one substitutes for the other: `scopedBy` alone lets any caller with `read` permission pick any project; `policies` alone doesn't understand "which project" at all. Use both together whenever a scope value should actually gate access, not just organize the result set. See [Permissions, roles, and ABAC policies](#permissions-roles-and-abac-policies) below for how RBAC and ABAC compose.
...
2. **Absence results in 403 on the response.** Omitting the scope header entirely does **not** let the write through unchecked. A JSON:API response echoes the record it just wrote, and that echo is a read subject to the same fail-closed scope filter as a plain `GET`. A `create`/`update` with no scope header therefore still comes back `403 Forbidden`, even though the mismatch check found nothing to reject and the row may already be written.

The practical rule: **every operation on a scoped entity needs the matching scope header, full stop.** There is no such thing as an unscoped write that "goes through untouched." It either matches the header (succeeds), contradicts it (400), or lacks it (403 on read-back). Scope is read context that the server checks going in and again coming out. It is not an ownership grant, and it will not silently repoint a relationship the way tenant scoping fills in `apertureTenantId`.

### Publishing `scopedBy` is a breaking change

Because the read filter is fail-closed, adding `scopedBy` to an entity that's already published (or repointing it at a different field) silently breaks every existing client that isn't yet sending the header. List calls start returning empty pages, while fetch-by-id and write read-backs start returning 403. The diff engine flags both as **breaking**, just like removing a field or changing its type, and requires an API version bump. Removing `scopedBy` only widens access and is classified as safe.
...
Turns on MCP tool generation project-wide. That is all the common case requires. The MCP tool surface is
...
model; it never extends it. There are two knobs, both optional, and both narrowing:
...
separate seams for adding MCP behavior: one at build time and one at runtime.
...
the per-entity ones, such as a tool that is not tied to any single entity.
...
runs inside the Maven build, long before the generated project's own JVM exists. There is no
...
entity tool classes. The result is an ordinary generated-project class with no reflection or
...
Implementations are configured explicitly under the Maven plugin's `<mcp><extensions>` list rather than discovered through ServiceLoader. The implementation class must be on the `aperture-maven-plugin`
...
different Elide setup. It mirrors the audit epic's `AuditWriter` seam: a well-known interface
